...
public class SummaryJsonWriter {
    ...
    public static SummaryPayload buildPayload(KafkaMessage message,
                                              ...
                                              List<PrintFile> printFiles, // <-- Deprecated
        ...
        if (processedFiles != null && !processedFiles.isEmpty()) {
            boolean allFailed = processedFiles.stream()
                    .allMatch(f -> "FAILURE".equalsIgnoreCase(Optional.ofNullable(f.getStatusCode()).orElse("")));
            boolean anyFailed = processedFiles.stream()
                    .anyMatch(f -> {
                        String code = Optional.ofNullable(f.getStatusCode()).orElse("");
                        return "FAILURE".equalsIgnoreCase(code) || "PARTIAL".equalsIgnoreCase(code);
                    });
        ...
        payload.setPrintFiles(null); // ❌ Removed printFiles block entirely
